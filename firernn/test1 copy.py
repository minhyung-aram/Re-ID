...
class LSTM(nn.Module):
    ...
    def forward(self, x):
       ...
       # result = self.relu(self.fc3(result))
        ...
        return final.view(-1, seq_length - encode_length, in_size, num_classes)
...
model = LSTM(input_size=in_size, num_layers=layers, hidden_size=hid_size).to(device)

lr_rate = 0.0005 # 0.001도 한번 해봐
optimizer = torch.optim.Adam(model.parameters(), lr=lr_rate)
loss_function = nn.CrossEntropyLoss()

num_epochs = 400 # 너무 적어 
epoch_loss = []
